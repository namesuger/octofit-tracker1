...
export default function Activities() {
  ...
  useEffect(() => {
    ...
    const loadActivities = async () => {
      try {
        ...
        if (isActive) {
          setActivities(getRecords(payload));
        }
      } catch (loadError) {
        if (isActive) {
  ...
  if (loading) {
    return <div className="route-panel"><p>Loading activities...</p></div>;
  }
  ...
  return (
    <div className="route-panel">
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h2>Activities</h2>
        <span className="badge text-bg-primary">{activities.length} items</span>
      </div>
            ...
            <tbody>
              {activities.map((activity) => (
                <tr key={activity._id ?? `${activity.type}-${activity.completedAt}`}>
                  {columns.map((column) => (
                    <td key={`${activity._id ?? activity.type}-${column}`}>{formatValue(activity[column])}</td>
                  ))}
                </tr>
              ))}
            </tbody>
